...
def pgf_plotter(direction: list, path: str) -> None:
    ...
    plt.title('Average Geostrophic Wind vs Time (1000hPa)')
    plt.ylabel(r'Velocity (m/s)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_1000.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['925hPa']))))
    plt.title('Average Geostrophic Wind vs Time (925hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_925.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['850hPa']))))
    plt.title('Average Geostrophic Wind vs Time (850hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_850.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['700hPa']))))
    plt.title('Average Geostrophic Wind vs Time (700hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_700.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['600hPa']))))
    plt.title('Average Geostrophic Wind vs Time (600hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_600.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['500hPa']))))
    plt.title('Average Geostrophic Wind vs Time (500hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_500.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['400hPa']))))
    plt.title('Average Geostrophic Wind vs Time (400hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_400.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['300hPa']))))
    plt.title('Average Geostrophic Wind vs Time (300hPa)')
    plt.ylabel(r'Velocity (m/s) ')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/geo_plots/geo_300.jpg')
    plt.show()
    plt.close('all')

...
def pgf_plotter(direction: list, path: str) -> None:
    ...
    for k in range(data.variables['hgt'].shape[0]):
        ...
        for i in range(np.shape(geo_data)[0]):
            ...
            bearing = np.abs(np.degrees(np.arctan(dpress_dy / dpress_dx)))  # Calculates PGF initial bearing
            if dpress_dx > 0 and dpress_dy > 0:
                degree = bearing  # Sets degree if quadrant 1
            if dpress_dx < 0 < dpress_dy:
    ...
    plt.title('Average PGF Direction vs Time (1000hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_1000.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['925hPa']))))
    plt.title('Average PGF Direction vs Time (925hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_925.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['850hPa']))))
    plt.title('Average PGF Direction vs Time (850hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_850.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['700hPa']))))
    plt.title('Average PGF Direction vs Time (700hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_700.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['600hPa']))))
    plt.title('Average PGF Direction vs Time (600hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_600.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['500hPa']))))
    plt.title('Average PGF Direction vs Time (500hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_500.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['400hPa']))))
    plt.title('Average PGF Direction vs Time (400hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_400.jpg')
    plt.show()
    plt.close('all')

    plt.plot(pgf_storage.Time, np.array(list(map(float, pgf_storage['300hPa']))))
    plt.title('Average PGF Direction vs Time (300hPa)')
    plt.ylabel(r'Direction ($\degree$)')
    plt.xlabel('Time (UTC)')
    plt.savefig('//uahdata/rstor/aes551_project_new/degree_plots/pgf_direction_300.jpg')
    plt.show()
    plt.close('all')
